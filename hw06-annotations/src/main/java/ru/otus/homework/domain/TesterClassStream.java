...
public class TesterClassStream {

    public static void runTesterStream(String classname) throws ClassNotFoundException {
        Class<?> aClass = Class.forName(classname);
        Method[] methods = aClass.getMethods();
        Method[] befores = Arrays.stream(methods)
        ...
        Arrays.stream(methods)
            ...
            Arrays.stream(befores).forEach(beforeMethod -> CheckedFunction1.of(beforeMethod::invoke).unchecked().apply(newInstance));
